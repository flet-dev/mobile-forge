...
- **The derivation in [`page.run_thread(...)`](https://flet.dev/docs/controls/page/#flet.Page.run_thread)**
  ...
  scrypt's 16 MB and racing to write the same file.
- **A wrong passphrase handled as an expected outcome** — `Fernet.decrypt` raises
  `InvalidToken`, caught and shown as `TextField.error`. Left uncaught in a worker thread it
