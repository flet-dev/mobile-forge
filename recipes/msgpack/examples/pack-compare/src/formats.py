...
def measure(obj):
    ...
    Returns one `(label, bytes, write_ms, read_ms, round_tripped)` tuple per
    format. A size printed without its round trip verified would be the one
    number on screen worth distrusting, so the last field is what the caller
    should say out loud.
...
def outcome(original, work):
    """Run one round trip and report it as a short cell of text.
    ...
    exists for. Long results are clipped so a phone-width column still fits.
    """
    try:
        restored = work()
...
def fidelity_rows():
    """What each format hands back for every case, as `(label, msgpack, json)`."""
    rows = []
