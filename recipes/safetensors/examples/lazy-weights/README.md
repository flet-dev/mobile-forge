...
One screen that writes a 48 MB weights file into app storage with numpy, then reads it four
ways and prints what each read costs in time and in peak resident memory. The claim being
tested is the reason to use
...
- **The header is free.** `safe_open` plus every tensor's name, shape and dtype takes well
  ...
  declares 48 MB of tensor data. The same line reports the identical header read done with
  nothing but `struct.unpack` and `json.loads`, because a model picker listing candidate
  files needs no more than that.
- **One row out of a 4 MiB tensor is free too** — `get_slice(name)[0:1]` on the block a
...
- **Truncation raises, bit rot does not.** The last button writes two damaged copies of a
  ...
  `SafetensorError: … incomplete metadata, file not fully covered`, and one with a single
  bit flipped inside a tensor, which opens perfectly and hands back the wrong number.
...
Two things to read correctly on screen. `ru_maxrss` is a **peak**, so the reading never
falls back — the ~48 MB the app allocated to build the file is already in the baseline that
stages 2 and 3 are compared against, and only stage 4 pushes past it. And the file stays in
[`FLET_APP_STORAGE_DATA`](https://flet.dev/docs/reference/environment-variables/#flet_app_storage_data)
between launches, as a weights file should; delete the app's data to reclaim the 48 MB.
