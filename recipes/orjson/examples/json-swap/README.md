...
swap is worth in absolute terms. Below that, a second table walks eight cases where the
swap is not transparent, every cell computed on the device rather than quoted.
...
- **The speedup, and how little it is worth at phone sizes.** The table shows `dumps` and
  ...
  slider that reads as a saving of a few microseconds, which is the honest answer and the
  reason the [recipe README](../../README.md) leads with output size and type coverage rather
  than with speed.
- **Two independent cross-checks, so a wrong answer is visible rather than merely
  ...
  printed as `identical bytes` or `DIFFERENT BYTES`. That second one is what makes the size
  row meaningful: it proves the smaller output is spacing and `\uXXXX` escaping rather than a
  different encoding.
...
- **The eight cases, each one a real call.** `{1: "a"}`, the same dict with
  ...
  raised, in its own column. The single most dangerous row is the 23-digit integer: `json`
  returns it exactly and orjson returns `1.2345678901234568e+22`, a float, with no error
  anywhere. Every one of those calls sits inside a broad `except Exception`, because they
  raise plain `TypeError`/`ValueError` rather than anything library-shaped and an unhandled
...
- **Where the native module came from, on this device.** The first header line carries
  ...
  which attribute survives varies by platform and by package — see the platform notes in the
...
- **Compute off the UI thread** — the run happens in
  ...
  Note that the thread buys nothing but a handler that returns immediately: orjson holds the
  GIL throughout, as the recipe README's [Threading](../../README.md#threading) section
  measures.
...
Nothing is downloaded, nothing is written to disk and no asset is bundled. numpy is left out
on purpose: `OPT_SERIALIZE_NUMPY` would be one more table row, and it would cost 6.5 MB of
wheel and 21 MB unpacked (measured on `numpy-2.4.6-1-cp314-cp314-android_24_arm64_v8a.whl`)
plus a `requires-python` bump to `>=3.11`, which is numpy's own floor. The recipe README
covers that flag in a bullet instead.
...
`pyproject.toml` pins both `flet` and `orjson`, which is the combination that was verified.
`requires-python` stays at `>=3.10`, orjson's own floor, so every split uv resolves for is
satisfiable — checked the way a consumer meets it, by copying that `pyproject.toml` alone
into an empty directory and running `uv lock` there.
