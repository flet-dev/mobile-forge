...
Everything that touches uuid-utils lives here, including the formatting of its
results, so that `main.py` only has to put finished strings into controls.
"""
...
# The app colours its header from this instead of importing uuid_utils itself.
LOADED = uuid_utils is not None
...
RATE_HEADER = ("source", "ns / id", "ids / second")

Report = namedtuple("Report", "rows summary ordering samples stamps window nodes")
...
def measure(scheme):
    """Time one scheme, then answer the four key questions about its batch.

    Returns finished text rather than ids: this is the slow call the app hands to
    a background thread, and keeping the formatting on this side of the split is
    what lets the worker in `main.py` stay a dozen lines. `ids` comes back None on
    a runtime that cannot generate the scheme at all — 3.12 and 3.13 without the
    wheel have no v6, v7 or v8 — and the four panels are then left empty rather
    than filled with numbers from the previous scheme.
    """
    rows, ids = rate_rows(scheme)
    if ids is None:
        return Report(
            rows=rows,
            summary=f"{scheme} · nothing on this runtime generates it",
            ordering="",
            samples=[],
            stamps="",
            window="",
    ...
    return Report(
        rows=rows,
        summary=f"{scheme} · {len(ids):,} ids · {distinct:,} distinct",
        ordering=order_line(ids, scheme),
        samples=[str(one) for one in ids[:SAMPLE_ROWS]] + [f"… {ids[-1]}"],
        stamps=stamp_line(ids, scheme),
        window=key_line(ids, scheme),
        nodes=node_identity(),
    )
