"""pyjnius is the Java<->Python bridge used for accessing the Android
runtime from Python. It can ONLY run on a real Android device — there's
no JVM/JNI environment on macOS or Linux CI runners. The test below
runs on the Android emulator job; iOS/local skips at import time."""
...
def test_jvm_classes():
    ...
    device's BRAND. This requires:
      - the libpyjnius .so to have loaded,
