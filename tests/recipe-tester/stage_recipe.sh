...
echo "  uv sync --dev --no-install-package $RECIPE  # host venv: skip recipe (mobile-only)"
echo "  PIP_FIND_LINKS=\"\$(realpath ../../dist)\" \\"
echo "    uv run --no-sync flet build apk --arch arm64-v8a"
